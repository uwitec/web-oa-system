 ...
 * 验证码 vcode
 */
public class ValidationCodeAction extends BaseAction implements
	...
	public String execute() throws Exception {
		...
		for (int i = 0; i < 4; i++) {
			...
			g.drawString(String.valueOf(codeChar), 13 * i + random.nextInt(7),
					height - random.nextInt(6));

		}
		HttpSession session = request.getSession();
		session.setMaxInactiveInterval(5 * 60); // 设置session对象5分钟失效
		// 将验证码保存在session对象中，key为validationCode
